...
-define(ARE_VALID_HD_NAME_SWAR(W),
...
-define(ARE_NOT_HASH_QMARK_SWAR(W),
	(W) band ?SWAR_MASK80 =:= 0 andalso
	?no_zero_byte((W) bxor 16#23232323232323) andalso
	?no_zero_byte((W) bxor 16#3F3F3F3F3F3F3F)
).

%% SWAR fast path: 7 bytes that are ASCII (< 128) and contain no $#.
-define(ARE_NOT_HASH_SWAR(W),
	(W) band ?SWAR_MASK80 =:= 0 andalso
	?no_zero_byte((W) bxor 16#23232323232323)
).

parse_fullpath(<<>>, Path) -> {Path, <<>>};
parse_fullpath(<< $#, _/bits >>, Path) -> {Path, <<>>};
parse_fullpath(<< $?, Qs/bits >>, Path) -> parse_fullpath_query(Qs, Path, <<>>);
parse_fullpath(<< W:56, Rest/bits >>, SoFar) when ?ARE_NOT_HASH_QMARK_SWAR(W) ->
...
parse_fullpath_query(<< W:56, Rest/bits >>, Path, SoFar) when ?ARE_NOT_HASH_SWAR(W) ->
